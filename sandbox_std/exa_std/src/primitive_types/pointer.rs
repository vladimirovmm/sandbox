...
fn test_ref() {
    ...
    unsafe {
        // После 5 выйдет за пределы размера строки. Ошибки не будет, но информация будет неверной.
        for _ in 0..10 {
    ...
    ref_a = a.as_ptr(); // Устанавливаем указатель на начало строки.
    ref_a = ref_a.wrapping_add(4); // сдвигаем указатель на 4 байта вперёд.
    unsafe {
        for n in 0..5 {
            // сдвигаем указатель на N байт назад и возвращаем значение. Указатель не перезаписываем. Т.е. он остаётся смотреть на 4 байта от начала
