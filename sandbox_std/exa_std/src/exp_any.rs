...
fn test_box_dyn() {
    ...
    /// Функция суммирует два значения разных типов. Второй аргумент - ссылка на значение любого типа.
    /// Эта функция лишь демонстрация работы с dyn Any. Понятнее и удобнее в использовании реализация через Trait.
    ...
    /// Разбита на отдельную функцию просто для демонстрации возможности и избежания повторения кода.
    fn multi_add2(back_result: isize, value: &dyn Any) -> isize {
        println!("type name by value: {}", std::any::type_name_of_val(value));

        if value.is::<String>() {
            println!("Строка: {}", value.downcast_ref::<String>().unwrap());
        }
